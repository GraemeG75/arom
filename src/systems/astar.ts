...
export class AStarPathfinder {
  public constructor(private readonly defaultMaxIterations: number = 5000) {}

  public findPath(start: Point, goal: Point, options: AStarSearchOptions): Point[] | undefined {
    const maxIterations: number = options.maxIterations ?? this.defaultMaxIterations;
    const isBlocked: (p: Point) => boolean = options.isBlocked ?? (() => false);
    const stepCost: AStarCostFn | undefined = options.stepCost;
